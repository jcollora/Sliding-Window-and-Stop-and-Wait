...
#define PORT "20630"     // my UDP port, note that it is not an integer, please change it to the last five digits of your student ID
...
int clientStopWait( UdpSocket &sock, const int max, int message[] ) {
  hwc4 func; return func.clientStopWait(sock, max, message);
}
int clientSlidingWindow( UdpSocket &sock, const int max, int message[], 
			  int windowSize ) {
  hwc4 func; return func.clientSlidingWindow(sock, max, message, windowSize);       
}
...
int main( int argc, char *argv[] ) {
  ...
  cerr << "   4: sliding windows with drops" << endl;
  cerr << "--> ";
  cin >> testNumber;
  ...
  if ( myPart == CLIENT ) {
    ...
    case 4:
      for(int windowSize = 1; windowSize <= 30; windowSize = windowSize + 29)
        for(int dropP = 0; dropP <= 10; dropP++) {
          cerr << "Elasped time = ";
          timer.start();
  ...
  if ( myPart == SERVER ) {
    ...
    case 4:
      for(int windowSize = 1; windowSize <= 30; windowSize = windowSize + 29)
        for(int dropP = 0; dropP <= 10; dropP++) {
          serverEarlyRetrans(sock, MAX, message, windowSize, dropP);
        }
      break;
    default:
      cerr << "no such test case" << endl;
